...
export function useTagAdmin() {
  ...
  const usageByTagId = useMemo(() => {
    ...
    for (const row of usageQuery.data ?? []) {
      // PostgREST returns this bigint as a JSON number, but coerce anyway: the
      // serialisation of bigint has varied across PostgREST versions and a
      // string here would sort lexicographically and break the usage ordering.
