...
-- PostgREST answers from a cached schema, and a relation missing from that
-- cache is a 404 rather than an error naming the cause. Supabase's DDL event
-- trigger normally reloads it; asking explicitly costs nothing and removes the
-- window where the view exists but the API still reports it missing.
