// ─── Venue time ─────────────────────────────────────────────────────────────
// Shared verbatim between the browser and the Supabase edge functions.
//
// This is policy, not container semantics: a file that records no UTC offset is
// stating local time at the camera, and deciding what "local" means is the
// library's call. If the client and the server ever disagreed about it, dates
// written by one would be silently wrong when read by the other — so the venue
