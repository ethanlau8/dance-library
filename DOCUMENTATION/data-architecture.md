...
- Declared `WITH (security_invoker = true)` so it runs as the caller and the existing `view_media`-gated RLS on `tags` and `media_tags` applies unchanged. Without it a view runs as its owner and would leak rows past RLS.
- `media_count` arrives as a string — PostgREST serialises `BIGINT` that way. Clients must coerce it.
